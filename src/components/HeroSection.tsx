  ...
  const cardData = [
    {
      ...
      description: "Create stunning, responsive websites with modern technologies. Built for performance, scalability, and user experience.",
      icon: "🌐",
      category: "Website",
    ...
    },
    {
      ...
      description: "Custom software development and enterprise solutions. Scalable, secure, and tailored to your business needs.",
      icon: "⚙️", 
      category: "Software",
    ...
    },
    {
      ...
      description: "Modern web applications with cutting-edge frameworks. Fast, interactive, and feature-rich experiences.",
      icon: "💻",
      category: "Web App",
  ...
  return (
    <>
      ...
      <section className="relative min-h-screen pb-26 flex flex-col items-center justify-center overflow-hidden bg-gradient-to-br from-gray-900 via-slate-900 to-gray-800 text-white">
        ...
        <div ref={heroSection.ref} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-16">
          {/* Hero Title */}
          <div className="space-y-8">
            <div className={`inline-flex px-6 py-3 bg-gradient-to-r from-green-500/20 to-cyan-500/20 border border-green-500/30 rounded-full text-green-400 text-sm font-medium backdrop-blur-sm transition-all duration-300 ${heroSection.inView ? 'opacity-100 scale-100' : 'opacity-0 scale-90'}`}>
              🟢 Smarter Idea, instant soultions
            </div>
            
            <h1 className="text-2xl md:text-3xl lg:text-5xl font-bold leading-tight">
              <div className={`transition-all duration-500 ease-out ${heroSection.inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
                We are your{' '}
                <span className="bg-gradient-to-r from-cyan-400 via-blue-400 to-green-400 bg-clip-text text-transparent animate-pulse">
                  Software
                </span>
              </div>
              <div className={`transition-all duration-500 ease-out delay-200 ${heroSection.inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'} flex items-center justify-center gap-4`}>
                Development 
                ...
                <span className="text-transparent bg-gradient-to-r from-cyan-400 to-green-400 bg-clip-text">Team</span>
              </div>
              <div className={`transition-all duration-500 ease-out delay-400 ${heroSection.inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
                in The Cloud
              </div>
            </h1>

            {/* Contact Button */}
            <div className={`transition-all pb-6 duration-500 ease-out delay-600 ${heroSection.inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          ...
          <div className="relative">
            <div className="relative mx-auto" style={{ width: '700px', height: '300px' }}>
              ...
              <div className="relative w-full h-full">
                {cardData.map((card, index) => (
                  <div
                    ...
                    style={{
                      transform: `translateY(${index === activeCard ? '0px' : `${(Math.abs(index - activeCard)) * 16}px`}) scale(${index === activeCard ? 1 : 0.92 - (Math.abs(index - activeCard) * 0.04)})`
                    }}
                  >
                    {/* Card Background with Gradient */}
                    <div className={`absolute inset-0 bg-gradient-to-br ${card.bgPattern} backdrop-blur-sm`}></div>
                    ...
                    <div className="relative z-10 p-8 h-full flex flex-col">
                      ...
                      <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-4">
                          <div className={`w-14 h-14 rounded-2xl bg-gradient-to-r ${card.color} flex items-center justify-center text-2xl shadow-lg`}>
                            {card.icon}
                          </div>
                          <div>
                            <h3 className="text-2xl font-bold text-white mb-1">{card.title}</h3>
                            <span className={`px-3 py-1 ${card.accent}/20 text-${card.accent.split('-')[1]}-400 rounded-full text-xs font-medium border border-${card.accent.split('-')[1]}-500/30`}>
                              {card.category}
                            </span>
                          </div>
                        </div>
